...
namespace engenious.Pipeline
{
    ...
    public class ModelProcessor : ContentProcessor<Assimp.Scene, ModelContent, ModelProcessorSettings>
    {
        ...
        {
            var n = new NodeContent(parent);
            model.Nodes.Add(n);
            if (_settings.TransformMesh)
            {
                var matrix = ConvertMatrix(node.Transform);
                matrix.M41 *= _settings.Scale.X;
                matrix.M42 *= _settings.Scale.Y;
                matrix.M43 *= _settings.Scale.Z;
        ...
        private static Matrix ConvertMatrix(Assimp.Matrix4x4 m)
        {
        ...
        public override ModelContent Process(Assimp.Scene scene, string filename, ContentProcessorContext context)
        {
            try
            {
                ...
                var des = c.GetSupportedExportFormats()[0];
                ...
                for (int meshIndex = 0; meshIndex < scene.MeshCount; meshIndex++)
                {
                    ...
                    foreach (var f in sceneMesh.Faces)
                    {
                        foreach (var i in f.Indices)
                        {
                            ...
                            if (sceneMesh.TextureCoordinateChannels.Length > 0 &&
                                sceneMesh.TextureCoordinateChannels[0].Count > i)
                            ...
                            meshContent.Vertices[vertex++] = new VertexPositionNormalTexture(
                                new Vector3(translated.X * _settings.Scale.X, translated.Y * _settings.Scale.Y,
                                    translated.Z * _settings.Scale.Z),
                ...
                foreach (var animation in scene.Animations)
                {
                    ...
                    foreach (var channel in animation.NodeAnimationChannels)
                    {
                        ...
                        int frameCount = Math.Max(Math.Max(channel.PositionKeyCount, channel.RotationKeyCount),
                            channel.ScalingKeyCount);
                        ...
                        for (int i = 0; i < frameCount; i++)
                        {
                            ...
                            //TODO: interpolation
                            var rot = channel.RotationKeyCount == 0
                                ? new Assimp.Quaternion(1, 0, 0, 0)
                                : i >= channel.RotationKeyCount
                                    ? channel.RotationKeys.Last().Value
                                    : channel.RotationKeys[i].Value;
                            var loc = channel.PositionKeyCount == 0
                                ? new Assimp.Vector3D()
                                : i >= channel.PositionKeyCount
                                    ? channel.PositionKeys.Last().Value
                                    : channel.PositionKeys[i].Value;
                            var sca = channel.ScalingKeyCount == 0
                                ? new Assimp.Vector3D(1, 1, 1)
                                : i >= channel.ScalingKeyCount
                                    ? channel.ScalingKeys.Last().Value
                                    : channel.ScalingKeys[i].Value;
                            rot.Normalize();

                            frame.Transform = new AnimationTransform(node.Node.Name,
