...
namespace engenious.Graphics
{
    public class RenderTarget2D : Texture2D
    {
            ...
            : base(graphicsDevice, width, height, 1, surfaceFormat)
        {
            ThreadingHelper.BlockOnUIThread(() =>
            {
                ...
                if (!isDepthTarget)
                {
                    ...
                    GL.RenderbufferStorage(RenderbufferTarget.Renderbuffer, (RenderbufferStorage) All.DepthComponent32,
                        width, height);
                }
        ...
        private void ErrorHandling()
        {
            switch (GL.CheckFramebufferStatus(FramebufferTarget.Framebuffer))
            {
                ...
                case FramebufferErrorCode.FramebufferUnsupportedExt:
                    throw new ArgumentException(
                        "FBO: This particular FBO configuration is not supported by the implementation.");
                default:
        ...
        {
            ThreadingHelper.BlockOnUIThread(() => { GL.DeleteFramebuffer(_fbo); });
            base.Dispose();
        }
    }
}