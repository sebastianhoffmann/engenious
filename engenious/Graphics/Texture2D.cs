...
namespace engenious.Graphics
{
    public class Texture2D : Texture
    {
            ...
            : base(graphicsDevice, 1, internalFormat)
        {
            ...
            ThreadingHelper.BlockOnUIThread(() =>
            {
                ...
                if (isDepthTarget)
                    ...
                    GL.TexStorage2D(TextureTarget2d.Texture2D, mipMaps, (SizedInternalFormat) internalFormat, width,
                        height);
        ...
        internal void SetData<T>(T[] data, int level,
            OpenTK.Graphics.OpenGL4.PixelFormat format = OpenTK.Graphics.OpenGL4.PixelFormat.Bgra) where T : struct
        //ValueType
        {
            bool hwCompressed = format == (OpenTK.Graphics.OpenGL4.PixelFormat) Content.TextureContentFormat.DXT1 ||
                                format == (OpenTK.Graphics.OpenGL4.PixelFormat) Content.TextureContentFormat.DXT3 ||
                                format == (OpenTK.Graphics.OpenGL4.PixelFormat) Content.TextureContentFormat.DXT5;
                ...
                ThreadingHelper.BlockOnUIThread(() =>
                {
                    ...
                    if (hwCompressed)
                    {
                        int blockSize = (format ==
                                         (OpenTK.Graphics.OpenGL4.PixelFormat) Content.TextureContentFormat.DXT1)
                            ? 8
                            : 16;
        ...
        public void GetData<T>(T[] data) where T : struct //ValueType
        {
            ...
            throw new NotImplementedException ();
                ...
                int level = 0;
                var rect = new Rectangle?(); //new Rectangle? (new Rectangle (0, 0, Width, Height));
                if (rect.HasValue)
                {
